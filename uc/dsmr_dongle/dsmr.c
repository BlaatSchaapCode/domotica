...
meterstand_t dsmr_parse(char *data, size_t size) {
	...
	// Initialise the floats to nan, so we can tell
	// a zero value from a missing value
	// clang-format off
	meterstand.phases[0].u_volt =
	meterstand.phases[0].i_ampere =
	meterstand.phases[0].e_received_kilowatt =
	meterstand.phases[0].e_delivered_kilowatt =
	meterstand.phases[0].p_receiving =
	meterstand.phases[0].p_delivering =
	meterstand.phases[1].u_volt =
	meterstand.phases[1].i_ampere =
	meterstand.phases[1].e_received_kilowatt =
	meterstand.phases[1].e_delivered_kilowatt =
	meterstand.phases[1].p_receiving =
	meterstand.phases[1].p_delivering =
	meterstand.phases[2].u_volt =
	meterstand.phases[2].i_ampere =
	meterstand.phases[2].e_received_kilowatt =
	meterstand.phases[2].e_delivered_kilowatt =
	meterstand.phases[2].p_receiving =
	meterstand.phases[2].p_delivering =
	meterstand.energy.hi_delivered_kilowatthour =
	meterstand.energy.hi_received_kilowatthour =
	meterstand.energy.lo_delivered_kilowatthour =
	meterstand.energy.lo_received_kilowatthour =
	meterstand.gas_m3 =
			NAN;
	// clang-format on
	...
	if (entry) {
		...
		if (entry) {
			...
			int count = sscanf(entry, "(%02d%02d%02d%02d%02d%02d%c",
					&meterstand.gas_time.year, &meterstand.gas_time.month,
					&meterstand.gas_time.day, &meterstand.gas_time.hour,
					&meterstand.gas_time.minute, &meterstand.gas_time.second,
	...
	if (entry) {
		entry = strstr(entry, "(");
		if (entry)
			meterstand.energy.lo_received_kilowatthour = strtof(entry + 1,
					NULL);
	}
	...
	if (entry) {
		entry = strstr(entry, "(");
		if (entry)
			meterstand.energy.hi_received_kilowatthour = strtof(entry + 1,
					NULL);
	}
	...
	if (entry) {
		entry = strstr(entry, "(");
		if (entry)
			meterstand.energy.lo_delivered_kilowatthour = strtof(entry + 1,
					NULL);
	}
	...
	if (entry) {
		entry = strstr(entry, "(");
		if (entry)
			meterstand.energy.hi_delivered_kilowatthour = strtof(entry + 1,
					NULL);
	}
	...
	if (entry) {
		...
		if (entry) {
			...
			meterstand.tariff = atoi(entry + 1);
		}
	}
